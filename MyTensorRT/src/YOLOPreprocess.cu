...
void YOLOPreprocess::cuda_preprocess(uint8_t* src, int src_width, int src_height, float* dst, int dst_width, int dst_height,
                     cudaStream_t stream) {
...
void YOLOPreprocess::cuda_batch_preprocess(cv::Mat& img, float* dst, int dst_width, int dst_height,
                           cudaStream_t stream) {
    cuda_preprocess(img.ptr(),img.cols, img.rows, dst, dst_width, dst_height, stream);
    CHECK(cudaStreamSynchronize(stream));
}

void YOLOPreprocess::cuda_preprocess_init(int max_image_size) {
...
void YOLOPreprocess::cuda_preprocess_destroy() {
    CHECK(cudaFree(img_buffer_device));
    CHECK(cudaFreeHost(img_buffer_host));
}
