...
void process(std::vector<Detection>& res, const float* decode_ptr_host,
    ...
    cv::Mat& img_ref = const_cast<cv::Mat&>(img);
    process_decode_ptr_host(res, decode_ptr_host, bbox_element, img_ref, count);
}
...
void draw_mask_bbox(cv::Mat& img, std::vector<Detection>& dets, std::vector<cv::Mat>& masks,
    ...
    for (size_t i = 0; i < dets.size(); i++) {
        ...
        cv::Size textSize =
                cv::getTextSize(labels_map[(int)dets[i].class_id] + " " + yolo11::to_string_with_precision(dets[i].conf),
        ...
        cv::putText(img, labels_map[(int)dets[i].class_id] + " " + yolo11::to_string_with_precision(dets[i].conf),
...
void draw_bbox_obb(std::vector<cv::Mat>& img_batch, std::vector<std::vector<Detection>>& res_batch) {
    ...
    for (size_t i = 0; i < img_batch.size(); i++) {
        ...
        for (auto& obj : res) {
            ...
            auto text = (std::to_string((int)(obj.class_id)) + ":" + yolo11::to_string_with_precision(obj.conf));
